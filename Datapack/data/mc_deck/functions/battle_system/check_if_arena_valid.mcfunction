...
execute if score first_arena_placed Debug matches 1.. run execute unless entity @e[tag=mc_deck.battle.arena.center,distance=..40] run function mc_deck:battle_system/create_arena